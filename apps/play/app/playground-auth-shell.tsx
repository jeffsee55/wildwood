...
async function getGithubAuthState(): Promise<{ email: string | null; githubSignedIn: boolean }> {
  ...
  try {
    ...
    return { email: session.user.email, githubSignedIn: accounts.some((a) => a.providerId === "github") };
  } catch {
...
export async function PlaygroundAuthShell({ initialConfig }: { initialConfig: PlaygroundConfig }) {
  ...
  playDebug("page.configLoaded", { activeRef: config.ref, config, githubSignedIn: githubAuth.githubSignedIn });
