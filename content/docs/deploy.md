...
Wildwood's deployment model keeps the hot path entirely on DB reads. Git checkout (local or GitHub API) is only needed at build and on incremental writes.

## Build prefetch

In dev and during `next build`, if a Git checkout exists (auto-detected or via `WILDWOOD_DOCS_REPO_PATH` / `localPath`), Wildwood uses `NativeRemote`. At query time, if the cache for `ref` is cold (no entries, no ref row), `findMany` → `ensureRefInDb(ref)` → `git.switch(ref)` traverses the git tree, reads blobs matching your collections, parses frontmatter/filters, records connections, and stores to LibSQL (the same DB driver you pass to `createClient`). Subsequent reads in the same build hit DB.

That DB — `file:./wildwood-docs.db` locally or `WILDWOOD_DOCS_DATABASE_URL` (Turso) — is the deployment artifact to populate. Vercel can run `next build` with the checkout available (your git repo clone for the build) and then you point runtime at Turso that already has the indexed rows.
...
const REF = process.env.WILDWOOD_DOCS_REF?.trim()
...
- During build (`NEXT_PHASE=phase-production-build`), `WILDWOOD_DOCS_SOURCE` is ignored for local auto-detection — the build host has a `.git` dir so it uses the NativeRemote, indexes to whatever LibSQL driver you passed. If you set `WILDWOOD_DOCS_DATABASE_URL=libsql://...` that directly writes to Turso at build; if `file:./wildwood-docs.db` you need a bundling step that pushes to Turso before deploy, or re-index in a post-deploy seed job.
...
WILDWOOD_DOCS_REF=main            # or ${VERCEL_GIT_COMMIT_SHA} on Vercel to pin SHA
WILDWOOD_DOCS_DATABASE_URL=libsql://...  # or file:./wildwood-docs.db for preview
...
Why `WILDWOOD_DOCS_REF` over `GIT_SHA`? One indirection lets you use `VERCEL_GIT_COMMIT_SHA` (Vercel automatically sets this) without coupling Wildwood to Vercel. Fallback order: `WILDWOOD_DOCS_REF` → `VERCEL_GIT_COMMIT_SHA` → `config.ref`.
...
The factory owns branch cookie + `revalidateTag`. The underlying H3 `handle(wildwood)` owns `/git/*`, `/wildwood/draft`, `/wildwood/preview`, `/vscode/*`, `/github/*`. No `next/*` inside handler — just `cookies()` via route factory outer layer.
...
To install Wildwood's GitHub App into a target repo for the editor's remote:
...
- Query caching via Next's `"use cache"` + `cacheTag(WILDWOOD_CACHE_TAG)` utilities (`cacheLife("hours")` typical). Enumerate in components, tag any function that reads `wildwood.docs.findMany`.
- `wildwood/nextjs/branch` exports `WILDWOOD_CACHE_TAG = "wildwood"` (`cacheTag` name). `createWildwoodRoute`'s mutation branch triggers `revalidateTag(tag, store)` for `commit | discard | merge | pull | create-branch | switch-branch` mutations (configurable via `mutationRe`).
...
- `WILDWOOD_DOCS_DATABASE_URL` driver is LibSQL which you can deploy at edge (Turso) — `createClient` accepts any `@libsql/client` (Node) or web `createClient` if you re-build for edge.

## Diagnostics

- `WILDWOOD_GIT_API_LOG=0` to silence `[wildwood:git-api]` logging.
- `[wildwood:git-add]` and `[wildwood:patch-worktree]` logs include ms timings for each phase (prep, blob/storage, tree application, persist, index).
...
WILDWOOD_DOCS_SOURCE=github WILDWOOD_DOCS_DATABASE_URL=file:./wildwood-docs.db pnpm --filter docs next build
...
# 2. Set runtime env
# WILDWOOD_DOCS_REF=$NEW_SHA (or rely on VERCEL_GIT_COMMIT_SHA)
# WILDWOOD_DOCS_DATABASE_URL, AUTH_TOKEN
# 3. Deploy
```

When `findMany` sans checkout in production issues "Wildwood index missing for ref X", it includes version + whether `WILDWOOD_DOCS_DATABASE_URL` / `LIBSQL_URL` / `TURSO_DATABASE_URL` presence hint, so you know instantly you missed the DB seed or the version bump.
