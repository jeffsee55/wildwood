...
- `localPath` — absolute path to a Git checkout. When set, wildwood uses a native Git remote (reads `.git` directly). When omitted, Wildwood auto-detects by walking up from `cwd` to `.git` in dev/build; in production it falls back to GitHub remote.
...
WILDWOOD_DOCS_REF=main           # or VERCEL_GIT_COMMIT_SHA
WILDWOOD_DOCS_DATABASE_URL=      # libsql://.. for Turso, or file:./tr33.db
