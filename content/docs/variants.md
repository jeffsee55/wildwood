...
- `findMostSpecificVariant(entries, variant)`):
   ...
   - Filter to *allowed*: entries whose every explicit axis matches the requested value. Default paths (`explicitKeys=[]`) always allowed. E.g. `b.fr.v2.md` allowed for `locale:fr|version:v2` but not for `locale:en|...` because `locale:fr != requested en`. Never fall back to an explicit-conflicting path.
