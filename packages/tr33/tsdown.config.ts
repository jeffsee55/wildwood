...
export default defineConfig([
  // ── main library (everything except client-boundary) ─────────────────
  {
    ...
    clean: true,
    exports: true,
    // `tr33-store` and `@tr33/shared` are pure ESM/JS — always bundle for
    // consumers. Do NOT bundle `@tr33/kit` — it has `'use client'` and must
    // stay a true peer so the consumer's bundler sees its directives.
    noExternal: ["tr33-store", "@tr33/shared"],
    external: [
      "next/headers",
      "next/cache",
      "next/server",
        ...
        for (const name of ["nextjs/client-boundary.js", "nextjs/client-boundary.mjs"]) {
          ...
          try {
            ...
            if (!src.startsWith("'use client'") && !src.startsWith('"use client"')) {
              ...
              console.info(`[tr33] ensured 'use client' in ${name}`);
            }
            break;
          } catch {
            // try next extension
