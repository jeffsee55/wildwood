...
const tr33GitApiLog = (...args: unknown[]) => {
  if (process.env.TR33_GIT_API_LOG === "0") return;
  console.info("[tr33:git-api]", ...args);
};

export function createGitServiceRouter(client: Tr33Client): H3 {
  ...
  router.get("/branches", async () => {
    try {
      ...
      for (const r of remoteRefs) if (!seen.has(r)) { seen.add(r); branches.push(r); }
      return Response.json({ branches });
    } catch (e) {
      console.error("Failed to list branches:", e);
      return new Response(`Failed to list branches: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  const checkEditorReady = async (refName: string) => {
    ...
    if (entryCount === 0) throw new Error(`Repository tree for "${refName}" is not indexed yet. Redeploy after build prefetch completes.`);
    return { ready: true as const, ref: refName, commitTreeOid: resolved.commit.treeOid, treeOid, entryCount };
  };
    ...
    try {
      ...
      if (installation.status === "not_installed") {
        return Response.json({
          status: "not_installed" as const, repo: repoFull,
  ...
  router.get("/editor-bootstrap", async (event) => {
    const { cookiesFromCookieHeader, resolveActiveRef } = await import("@/nextjs/resolve-active-ref");
    const { resolveVscodeWebCdn } = await import("@/nextjs/vscode-web-cdn");
    const cookies = cookiesFromCookieHeader(event.req.headers.get("cookie"));
    const refName = resolveActiveRef({ tr33: client, cookies } as never);
    try {
  ...
  router.post("/switch-branch", async (event) => {
    try {
      ...
      return new Response(`Failed to switch branch: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/create-branch", async (event) => {
    try {
      ...
      return new Response(`Failed to create branch: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/worktrees/:ref", async (event) => {
    ...
    try {
      ...
      tr33GitApiLog("GET /worktrees/:ref", { ref: refName, org, repo, url: event.url?.href ?? refParam });
      const authError = await authorizeGitAction(client, event.req, { type: "git.switchRef", ref: refName });
      if (authError) return authError;
      await client._.db.init();
      const resolved = await git.resolveWorktreeForApi({ ref: refName });
      tr33GitApiLog("GET /worktrees — ok (read)", { ref: refName, commitTree: resolved.commit.treeOid.slice(0, 7), hasRootTree: Boolean(resolved.rootTreeOid) });
      return Response.json(resolved);
    } catch (e) {
      console.error("Failed to fetch worktree:", e);
      return new Response(`Failed to fetch worktree: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/tree/:oid", async (event) => {
    ...
    try {
      tr33GitApiLog("GET /tree/:oid", { oid: oid.slice(0, 7), org, repo });
      const treeEntry = await git.getTree(oid);
      if (!treeEntry) {
        tr33GitApiLog("GET /tree/:oid — not found", { oid: oid.slice(0, 7) });
        return new Response("Tree not found", { status: 404 });
      }
      const entryCount = Object.keys(treeEntry).length;
      tr33GitApiLog("GET /tree/:oid — ok", { oid: oid.slice(0, 7), entryCount });
      if (entryCount === 0) console.warn("[tr33:git-api] GET /tree/:oid — empty tree", { oid: oid.slice(0, 7), org, repo });
      return Response.json(treeEntry, { headers: gitObjectCacheHeaders(oid) });
    } catch (e) {
      console.error("Failed to fetch tree:", e);
      return new Response(`Failed to fetch tree: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/blob/:oid", async (event) => {
    ...
    try {
      ...
      if (localBlobs.length > 0) {
        return Response.json({ type: "blob", oid: localBlobs[0]!.oid, content: localBlobs[0]!.content }, { headers: gitObjectCacheHeaders(oid) });
      }
      const blobs = await remote.fetchBlobs({ oids: [oid] });
      if (blobs.length === 0) return new Response("Blob not found", { status: 404 });
      return Response.json({ type: "blob", oid: blobs[0]!.oid, content: blobs[0]!.content }, { headers: gitObjectCacheHeaders(oid) });
    } catch (e) {
      console.error("Failed to fetch blob:", e);
      return new Response(`Failed to fetch blob: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/blob/:oid/raw", async (event) => {
    ...
    try {
      const localBlobs = await client._.git.db.blobs.batchGet({ oids: [oid] });
      if (localBlobs.length > 0) {
        return new Response(new TextEncoder().encode(localBlobs[0]!.content), { headers: { "Content-Type": "application/octet-stream", ...gitObjectCacheHeaders(oid) } });
      }
      const raw = await remote.fetchBlobRaw({ oid });
      if (!raw) return new Response("Blob not found", { status: 404 });
      return new Response(new Uint8Array(raw), { headers: { "Content-Type": "application/octet-stream", ...gitObjectCacheHeaders(oid) } });
    } catch (e) {
      console.error("Failed to fetch raw blob:", e);
      return new Response(`Failed to fetch raw blob: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/commit/:oid", async (event) => {
    ...
    try {
      ...
      return new Response(`Failed to fetch commit: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/merge-base/:ours/:theirs", async (event) => {
    ...
    if (!ours || !theirs) return new Response("Both ours and theirs params required", { status: 400 });
    try {
      ...
      if (oursCommit.oid === theirsCommit.oid) return Response.json({ oid: oursCommit.oid, treeOid: oursCommit.treeOid });
      const mergeBase = await git.trees.findMergeBase({ oursOid: oursCommit.oid, theirsOid: theirsCommit.oid });
      ...
      return new Response(`Failed to compute merge base: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/add", async (event) => {
    try {
      const { ref: refParam, files: filesParam, stream: streamProgress } = z.object({
        ref: z.string(),
      ...
      if (streamProgress) {
        ...
        const bodyStream = new ReadableStream<Uint8Array>({
          async start(controller) {
            const emit = (p: Record<string, unknown>) => controller.enqueue(encoder.encode(`${JSON.stringify(p)}\n`));
            try {
              const addResult = await client._.git.add({ ref: refParam, files, onProgress: (m) => emit({ type: "progress", message: m }) });
              emit({ type: "done", ref: refParam, files: addResult.files, rootTreeOid: addResult.rootTreeOid });
            } catch (error) {
              emit({ type: "error", message: error instanceof Error ? error.message : String(error) });
            } finally { controller.close(); }
          },
        });
        return new Response(bodyStream, { headers: { "Content-Type": "application/x-ndjson", "Cache-Control": "no-store" } });
      }

      const addResult = await client._.git.add({ ref: refParam, files });
      return Response.json({ ref: refParam, files: addResult.files, rootTreeOid: addResult.rootTreeOid });
    } catch (e) {
      console.error("Failed to add files:", e);
      return new Response(`Failed to add files: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/patch-worktree", async (event) => {
    try {
      ...
      return new Response(`Failed to patch worktree: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/commit", async (event) => {
    try {
      ...
      return new Response(`Failed to commit: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/discard", async (event) => {
    try {
      ...
      return new Response(`Failed to discard: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/push", async (event) => {
    try {
      ...
      return new Response(`Failed to push: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/pull", async (event) => {
    try {
      ...
      await git.db.refs.updateCommit({ ref: refParam, commit: (pullResult as never as { commit: never }).commit });
      await git.db.refs.setTreeOid({ ref: refParam, treeOid: successResult.commit.treeOid });
      return Response.json({ ok: true, commitOid: (pullResult as { commit: { oid: string } }).commit.oid });
    } catch (e) {
      console.error("Failed to pull:", e);
      return new Response(`Failed to pull: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/merge", async (event) => {
    try {
      ...
      try {
        ...
        if (!pr) {
          pr = await remote.createPr({ head: refParam, base: configRef, title: `Merge ${refParam} into ${configRef}`, body: `Auto-created by Tr33 for ${refParam} -> ${configRef}.` });
        }
        ...
        try {
          ...
          if ((pullResult as { type: string }).type === "success") {
            await git.db.refs.updateCommit({ ref: configRef, commit: (pullResult as never as { commit: never }).commit });
            await git.db.refs.setTreeOid({ ref: configRef, treeOid: (pullResult as { commit: { treeOid: string } }).commit.treeOid });
          }
        } catch (pullError) {
          console.error(`[gitService] PR merged successfully but local pull failed for ${configRef}:`, pullError);
        }
        return Response.json({ ok: true, pr: { number: pr.number, url: pr.url }, commitOid: mergeResult.commitOid });
      } catch (error) {
        if (!isNativeRemoteNotImplementedError(error)) throw error;
        console.warn(`[gitService] PR operations are not implemented for NativeRemote; falling back to local merge (${refParam} -> ${configRef})`, error);
        const localMerge = await git.merge({ ours: configRef, theirs: refParam, message: messageParam } as never);
        if ((localMerge as { type: string }).type === "conflict") {
          return new Response(`Failed to merge: local merge conflict for ${refParam} -> ${configRef}`, { status: 409 });
        }
        await git.db.refs.updateCommit({ ref: configRef, commit: (localMerge as never as { commit: never }).commit });
        await git.db.refs.setTreeOid({ ref: configRef, treeOid: (localMerge as { commit: { treeOid: string } }).commit.treeOid });
        return Response.json({ ok: true, pr: null, commitOid: (localMerge as { commit: { oid: string } }).commit.oid });
      }
    } catch (e) {
      console.error("Failed to merge:", e);
      return new Response(`Failed to merge: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.post("/create-pr", async (event) => {
    try {
      ...
      if (!pr) {
        ...
        const defaultBody = `Auto-created by Tr33 for ${refParam} -> ${configRef}.`;
        const rawTitle = titleParam?.trim() || defaultTitle;
        const MAX_TITLE = 128;
        const title = rawTitle.length > MAX_TITLE ? `${rawTitle.slice(0, MAX_TITLE)}…` : rawTitle;
        pr = await remote.createPr({ head: refParam, base: configRef, title, body: bodyParam?.trim() || defaultBody });
      }
      return Response.json({ ok: true, pr: { number: pr.number, url: pr.url } });
    } catch (e) {
      console.error("Failed to create PR:", e);
      return new Response(`Failed to create PR: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
  ...
  router.get("/pr/:ref", async (event) => {
    try {
      ...
      return new Response(`Failed to find PR: ${e instanceof Error ? e.message : String(e)}`, { status: 500 });
    }
  });
