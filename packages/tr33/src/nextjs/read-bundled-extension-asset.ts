...
export function getTr33PackageRoot(): string {
  return dirname(nodeRequire.resolve("tr33/package.json"));
}

function extensionAssetRoots(): string[] {
  const roots = [
    join(getTr33PackageRoot(), "bundled-extension"),
    join(process.cwd(), "packages", "tr33", "bundled-extension"),
    join(process.cwd(), "node_modules", "tr33", "bundled-extension"),
    join(process.cwd(), "packages", "extension"),
    join(process.cwd(), "..", "..", "packages", "extension"),
    join(process.cwd(), "node_modules", "tr33-vscode"),
  ];

  try {
    roots.push(dirname(nodeRequire.resolve("tr33-vscode/package.json")));
  } catch {
    /* optional */
  }
