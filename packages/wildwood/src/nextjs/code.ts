import { WILDWOOD_ACTIVE_REF_STORAGE_KEY } from "wildwood-shared";
...
<html>
  ...
  <script type="module">
    ...
    try {
      const storedRef = localStorage.getItem(${JSON.stringify(WILDWOOD_ACTIVE_REF_STORAGE_KEY)});
      if (typeof storedRef === "string" && storedRef.trim().length > 0) {
        config.configurationDefaults = config.configurationDefaults ?? {};
        config.configurationDefaults["wildwood.headRef"] = storedRef.trim();
      }
    } catch {
