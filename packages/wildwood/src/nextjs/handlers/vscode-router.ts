...
export function createVscodeRouter(client: WildwoodClient): H3 {
  ...
  const getWorkbenchConfig = async (event: H3EventLite) => {
    ...
    return {
      ...
      configurationDefaults: {
        "workbench.colorTheme": "Wildwood Dark",
        "wildwood.repo": repoFull,
        "wildwood.headRef": ref,
        "wildwood.baseRef": ref,
        "workbench.editorAssociations": {
          "*.png": "wildwood.imagePreview",
          "*.jpg": "wildwood.imagePreview",
          "*.jpeg": "wildwood.imagePreview",
          "*.gif": "wildwood.imagePreview",
          "*.webp": "wildwood.imagePreview",
          "*.svg": "wildwood.imagePreview",
          "*.bmp": "wildwood.imagePreview",
          "*.ico": "wildwood.imagePreview",
        },
      },
    } as const;
  };
  ...
  const serveWildwoodExtensionAsset = async (event: H3EventLite, asset: string) => {
    ...
    try {
      ...
      if (asset === "package.nls.json") {
        ...
        return withVscodeEmbedCors(
          event.req,
          Response.json(extensionNls as unknown as Record<string, unknown>, {
      ...
      return new Response(
        `Failed to load extension asset: ${e instanceof Error ? e.message : String(e)}`,
        { status: 500, headers: vscodeEmbedCorsHeaders(event.req) },
      );
    }
  };
  ...
  const serveEditorHtml = async (event: any, commitParam?: string) => {
    const cdn = await resolveVscodeWebCdn();
    if (commitParam && commitParam !== cdn.commit)
      return new Response("VS Code commit mismatch", { status: 404 });
    const wc = await getWorkbenchConfig(event as H3EventLite);
    const code = getCode({
      origin: resolveEventOrigin(event as H3EventLite),
      prefix: "/api/vscode",
      workbenchConfig: wc as never,
      vscodeWebCdn: cdn as never,
    });
  ...
  vscode
    .get("/editor/:commit", async (event) => {
      const cp = routeParamString(
        (event.context.params as Record<string, unknown>)?.commit as string | number | undefined,
      );
    ...
    .get("extensions/**:asset", async (event) => {
      const asset = routeParamPath(
        (event.context.params as Record<string, unknown>)?.asset as
          | string
          | string[]
          | number
          | undefined,
      ...
      return (proxyMainVscodeCdnAsset as unknown as (req: unknown, p: string) => unknown)(
        event.req as unknown,
        `extensions/${asset}`,
      ) as never;
    })
    .get("/extension/package.json", async (event) =>
      serveWildwoodExtensionAsset(event as H3EventLite, "package.json"),
    )
  ...
  vscodeCdn.get("/:commit/**:asset", async (event) => {
    ...
    const commitParam = routeParamString(
      (event.context.params as Record<string, unknown>)?.commit as string | number | undefined,
    );
  ...
  vscodeCdn.get("/", async (event) => {
    ...
    return Response.redirect(
      `${resolveEventOrigin(event as unknown as H3EventLite)}${base}/${cdn.commit}/out/vs/code/browser/workbench/workbench.html`,
      302,
    );
  });
