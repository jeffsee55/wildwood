...
export function createGitServiceRouter(
  ...
  router.get("/editor-bootstrap", async (event) => {
    const { cookiesFromCookieHeader, resolveActiveRef, resolveBranch, WILDWOOD_BRANCH_COOKIE_FALLBACKS } = await import("@/nextjs/resolve-active-ref");
    const { resolveVscodeWebCdn } = await import("@/nextjs/vscode-web-cdn");
    ...
    if (!refName) {
      refName = resolveActiveRef({ wildwood: client as never, cookies, cookieName: undefined } as unknown as never);
  ...
  router.get("/tree/:oid", async (event) => {
    ...
    try {
      ...
      if (entryCount === 0) console.warn("[wildwood:git-api] GET /tree/:oid — empty tree", { oid: oid.slice(0, 7), org, repo });
  ...
  router.post("/merge", async (event) => {
    try {
      ...
      try {
        ...
        if (!pr) {
          pr = await remote.createPr({ head: refParam, base: configRef, title: `Merge ${refParam} into ${configRef}`, body: `Auto-created by Wildwood for ${refParam} -> ${configRef}.` });
        }
